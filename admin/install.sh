...
if [ -e "$DIR_ROOT" ]; then
  if [ -d "$DIR_ROOT" ]; then
    echo $styleInfo"Required '$DIR_ROOT' path already exists and is a folder"$styleEnd
  else
    echo $styleInfo"Required '$DIR_ROOT' path already exists and is a file"$styleEnd
  fi
